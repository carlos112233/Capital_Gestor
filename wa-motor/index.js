...
if (isMysql) {
    ...
    const mysqlHost =
        process.env.DB_HOST === "localhost" || !process.env.DB_HOST
            ? "127.0.0.1"
            : process.env.DB_HOST;
    ...
    console.log(
        `🗄️ Inicializado pool MySQL para host "${mysqlHost}" en base de datos "${process.env.DB_DATABASE || "gestor_capital_db"}"`,
    );
} else {
...
function probarConexionBaseDatos() {
    const testQuery = isMysql
        ? "SELECT DATABASE() as current_db_name, USER() as current_user_name"
        : "SELECT current_database(), current_user";
    queryDb(testQuery)
        .then((res) => {
            const dbName =
                res.rows[0]?.current_database ||
                res.rows[0]?.current_db_name ||
                res.rows[0]?.["DATABASE()"] ||
                process.env.DB_DATABASE;
            const dbUser =
                res.rows[0]?.current_user ||
                res.rows[0]?.current_user_name ||
                res.rows[0]?.["USER()"] ||
                process.env.DB_USERNAME;
            const hostTarget = isMysql
                ? process.env.DB_HOST || "127.0.0.1"
                : process.env.DATABASE_URL
                  ? "Render Cloud PostgreSQL"
                  : process.env.DB_HOST || "127.0.0.1";
            ...
            console.log(
                `🗄️ Conexión exitosa a la Base de Datos (${isMysql ? "MySQL" : "PostgreSQL"}): "${dbName}" en [${hostTarget}] como usuario "${dbUser}"`,
            );
        })
        .catch((err) => {
            console.error(
                `❌ Error de conexión a la base de datos (${isMysql ? "MySQL" : "PostgreSQL"}):`,
                err.message,
            );
            ...
            guardarEstado(
                ...
                `Error de conexión a la Base de Datos ${isMysql ? "MySQL" : "PostgreSQL"}`,
                {
                    ...
                    solution_hint:
                        "Verifica la variable DATABASE_URL en Render o las variables DB_USERNAME / DB_PASSWORD en el archivo .env.",
                },
            );
        });
}
...
function guardarEstado(estado, mensaje, opciones = {}) {
    ...
    try {
        const publicStatusPath = path.join(
            __dirname,
            "..",
            "public",
            "wa-status.json",
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (item.isDirectory()) {
                ...
                item.name.includes("Singleton") ||
                item.name === "DevToolsActivePort"
            ) {
                try {
                    fs.unlinkSync(fullPath);
                    console.log(
                        `🧹 Lockfile de Chromium eliminado automáticamente: ${item.name}`,
                    );
                } catch (e) {}
            }
        }
...
function findChromePath() {
    ...
    try {
        const found = execSync(
            "which chromium || which chromium-browser || which google-chrome || which google-chrome-stable 2>/dev/null",
            { encoding: "utf8" },
        ).trim();
...
if (!chromePath) {
    console.log(
        "⏳ No se detectó navegador en el sistema. Descargando Chromium automáticamente...",
    );
    guardarEstado(
        "cargando",
        "Navegador no encontrado en el sistema. Intentando instalación automática de Chromium...",
        {
            error_type: "Navegador",
            solution_hint:
                "Si la descarga falla, instala Chromium manualmente en tu servidor Linux (apt install -y chromium).",
        },
    );
    ...
    try {
        ...
        console.error(
            "Error al descargar navegador automáticamente:",
            e.message,
        );
        guardarEstado(
            "error",
            "No se pudo iniciar el navegador Chromium en el servidor.",
            {
                error_type: "Navegador",
                detail: e.message,
                solution_hint:
                    "Ejecuta en la consola de tu servidor Linux: apt update && apt install -y chromium",
            },
        );
    }
}
...
console.log(
    `🌐 Navegador detectado para WhatsApp: ${chromePath || "Chromium nativo de Puppeteer"}`,
);
guardarEstado(
    "cargando",
    "Iniciando navegador Chromium y conectando a WhatsApp Web...",
);
...
function limpiarQRArchivos() {
    try {
        ...
        const brainPath =
            "/home/araiza/.gemini/antigravity-ide/brain/20e4ef77-10df-4090-9fac-89a163a822e6/qr.png";
        ...
        console.log(
            "🧹 QR eliminado automáticamente porque la sesión ya se encuentra vinculada y activa.",
        );
    } catch (e) {
...
client.on("qr", async (qr) => {
    ...
    try {
        ...
        guardarEstado(
            "qr_pendiente",
            "Código QR listo para escanear. Abre WhatsApp en tu celular > Dispositivos vinculados.",
        );
    } catch (e) {
        console.error("Error guardando imagen QR:", e.message);
        guardarEstado(
            "error",
            "No se pudo guardar la imagen del código QR en el servidor.",
            {
                error_type: "Generación de QR",
                detail: e.message,
                solution_hint:
                    "Verifica que el servidor tenga permisos de escritura en la carpeta public/img.",
            },
        );
    }
});
...
function arrancarBucle() {
    if (!bucleIniciado) {
        bucleIniciado = true;
        console.log(
            "🚀 Bucle de procesamiento de mensajes de WhatsApp activado.",
        );
        iniciarBucleEnvio();
    }
}
...
client.on("authenticated", () => {
    ...
    guardarEstado(
        "conectado",
        "WhatsApp vinculado y activo en el sistema. Dispositivo autenticado.",
    );
    limpiarQRArchivos();
    arrancarBucle();
});
...
client.on("ready", () => {
    console.log(
        "🚀 MOTOR LISTO: WhatsApp está conectado y listo para enviar mensajes.",
    );
    guardarEstado(
        "conectado",
        "WhatsApp vinculado y activo en el sistema. Dispositivo autenticado.",
...
client.on("auth_failure", (msg) => {
    ...
    guardarEstado(
        "error",
        "Falló la autenticación con WhatsApp. La sesión expiró o fue revocada.",
        {
            error_type: "Autenticación",
            detail: typeof msg === "string" ? msg : JSON.stringify(msg),
            solution_hint:
                'Haz clic en "Cerrar Sesión & Nuevo QR" para borrar la sesión anterior e intentar de nuevo.',
        },
    );
});
...
client.on("disconnected", (reason) => {
    ...
    guardarEstado(
        "desconectado",
        "La sesión fue desconectada del dispositivo móvil.",
        {
            error_type: "Desconexión",
            detail:
                typeof reason === "string" ? reason : JSON.stringify(reason),
            solution_hint:
                "Comprueba que tu celular tenga conexión a internet y vuelve a vincular el dispositivo.",
        },
    );
...
process.on("uncaughtException", (err) => {
    ...
    if (
        detailStr.includes("The browser is already running") ||
        detailStr.includes("SingletonLock") ||
        detailStr.includes("WS endpoint") ||
        detailStr.includes("Timed out after")
    ) {
        removerLockFiles(path.join(__dirname, ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", "public", ".wwebjs_auth"));
        ...
        guardarEstado(
            "cargando",
            "Iniciando WhatsApp Web (Tiempo de carga extendido en servidor)...",
        );
        setTimeout(
            () =>
                client
    ...
    guardarEstado(
        "error",
        "Ocurrió una excepción no controlada en el motor de WhatsApp.",
        {
            error_type: "Excepción del Sistema",
            detail: detailStr,
            solution_hint:
                'Haz clic en "Cerrar Sesión & Nuevo QR" para reiniciar el motor de notificaciones.',
        },
    );
});
...
process.on("unhandledRejection", (reason, promise) => {
    ...
    const detailStr = reason
        ? reason.stack || reason.message || String(reason)
        : "Rechazo desconocido";
    const lowerDetail = detailStr.toLowerCase();

    if (
        lowerDetail.includes("the browser is already running") ||
        lowerDetail.includes("singletonlock") ||
        lowerDetail.includes("ws endpoint") ||
        lowerDetail.includes("timed out after")
    ) {
        removerLockFiles(path.join(__dirname, ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", "public", ".wwebjs_auth"));
    ...
    if (
        lowerDetail.includes("auth timeout") ||
        lowerDetail.includes("qr read timeout") ||
        lowerDetail.includes("protocolerror")
    ) {
        removerLockFiles(path.join(__dirname, ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", ".wwebjs_auth"));
        removerLockFiles(path.join(__dirname, "..", "public", ".wwebjs_auth"));
        console.log(
            "🔄 Reintento automático por expiración del QR / Auth Timeout...",
        );
        guardarEstado(
            "cargando",
            "Generando automáticamente nuevo código QR (Expiró el tiempo del código anterior)...",
        );
        setTimeout(
            () =>
                client
                    ...
                    .catch((e) =>
                        console.error(
                            "Error al reinicializar cliente tras auth timeout:",
                            e,
                        ),
                    ),
            3000,
    ...
    guardarEstado(
        "error",
        "Error en promesa asíncrona del motor de WhatsApp.",
        {
            error_type: "Promesa Asíncrona",
            detail: detailStr,
            solution_hint:
                'Reinicia la sesión con el botón de "Cerrar Sesión & Nuevo QR".',
        },
    );
});
...
function iniciarBucleEnvio() {
    setInterval(async () => {
        ...
        try {
            ...
            if (res.rows && res.rows.length > 0) {
                ...
                for (const msg of res.rows) {
                    try {
                        ...
                        let cleanNum = rawNum.includes("@c.us")
                            ? rawNum.replace("@c.us", "")
                            : rawNum;
                        ...
                        if (!cleanNum) {
                            console.error(
                                `⚠️ Número inválido en mensaje #${msg.id}`,
                            );
                        ...
                        try {
                            ...
                            console.log(
                                `⚠️ No se pudo obtener ID verificado para ${cleanNum}, usando destino por defecto ${targetId}`,
                            );
                        }
            ...
            isProcessing = false; // Abre el candado al terminar
        }
    }, 5000);
}
...
client
    .initialize()
    .catch((e) => console.error("Error al inicializar cliente:", e));
