...
class CobroController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pedidosQuery = Pedido::with(['user', 'articulo', 'venta'])->latest();
        if ($search) {
            $searchLower = '%' . strtolower($search) . '%';
        ...
        // Calcular Métricas KPI exactas
        $totalCobrado = $todosLosRegistros->where('estado', 'PAGADO')->sum('monto');
        $inicioMes = now()->startOfMonth();
        $cobrosMes = $todosLosRegistros->where('estado', 'PAGADO')->filter(function($item) use ($inicioMes) {
