        ...
        <tbody>
            @forelse($movimientos as $mov)
                <tr>
                    ...
                    <td style="text-align: left;">{{ $mov->articulo->nombre ?? $mov->concepto ?? $mov->descripcion ?? 'Consumo Restaurante' }}{{ isset($mov->cantidad) && $mov->cantidad > 1 ? " (x{$mov->cantidad})" : '' }}</td>
                    <td style="text-align: center;">#{{ $mov->id }}</td>
                    <td style="text-align: right; font-weight: 600;">${{ number_format($mov->monto ?? $mov->total_venta ?? $mov->precio_venta, 2) }}</td>
                </tr>
            @empty
                <tr>
