<x-app-layout>
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs mb-6">
        <form method="GET" action="{{ route('cobros.index') }}" class="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <!-- Campo de Búsqueda -->
            <div class="relative z-0 w-full md:w-2/3 group">
    <input type="text" name="q" id="search_q" value="{{ request('q') }}" class="block py-2.5 px-0 w-full text-sm text-slate-800 bg-transparent border-0 border-b-2 border-slate-300 appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 peer" placeholder=" " autocomplete="off" />
    <label for="search_q" class="peer-focus:font-medium absolute text-sm text-slate-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-indigo-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 flex items-center gap-2 cursor-text">
        ...
        Buscar por usuario, correo, folio o concepto...
    </label>
</div>
