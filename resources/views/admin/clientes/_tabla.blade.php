            ...
            <tr>
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-gray-500">
                    <div class="flex items-center justify-center gap-2">
                        <img src="{{ route('user.image', $cliente->id) }}?v={{ $cliente->updated_at ? $cliente->updated_at->timestamp : '' }}"
                            class="w-7 h-7 rounded-full object-cover border border-slate-200 shadow-sm" alt="Foto">
                    ...
                    <div class="flex items-center justify-center gap-3">
                        ...
                        @if ($cliente->trashed())
                            ...
                            <form id="activate-cliente-{{ $cliente->id }}" class="inline-block"
                                ...
                                @csrf
                                <button type="submit"
                                    class="text-green-600 hover:text-green-900 font-semibold cursor-pointer">
                                    Activar
                                </button>
                            </form>
                        @else
                            {{-- Botón Eliminar con SweetAlert2 --}}
                            <form id="delete-cliente-{{ $cliente->id }}" class="contents"
                        ...
                        <x-modal name="wa-cliente-{{ $cliente->id }}">
                            <div class="p-6 text-left">
                                <div class="flex justify-between items-center pb-3 border-b mb-4">
                                    ...
                                    <button type="button" onclick="closeModal('wa-cliente-{{ $cliente->id }}')"
                                ...
                                <form method="POST"
                                    action="{{ route('admin.clientes.enviar-whatsapp', $cliente->id) }}">
                                    @csrf
                                    <div class="mb-4">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Si ingresas un texto aquí, la contraseña
                                            del cliente se actualizará a este valor y se enviará por WhatsApp.</p>
                                    </div>

                                    <div class="flex justify-end gap-3 mt-6">
                                        <button type="button" onclick="closeModal('wa-cliente-{{ $cliente->id }}')"
                                            class="px-4 py-2 rounded-xl border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 text-sm font-semibold cursor-pointer">
                                            Cancelar
                                        </button>
                                        <button type="submit"
                                            class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-sm shadow-md shadow-emerald-500/20 hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer">
                    ...
                    <x-modal name="edit-cliente-{{ $cliente->id }}">
                        <div class="p-6 text-left">
                            <div class="flex justify-between items-center pb-3 border-b mb-4">
                                ...
                                <button type="button" onclick="closeModal('edit-cliente-{{ $cliente->id }}')"
                                    class="text-gray-400 hover:text-gray-600 font-bold text-xl">&times;</button>
                            </div>
            ...
            <tr>
                <td colspan="7" class="px-4 py-8 border-b text-center text-gray-500 bg-gray-50/50">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        @if (request()->filled('q'))
                            <span class="font-medium text-gray-700">No se encontraron clientes que coincidan con
                                "{{ request()->q }}"</span>
