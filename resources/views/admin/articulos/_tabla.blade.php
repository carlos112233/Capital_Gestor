  <table class="min-w-full divide-y divide-gray-200">
      <thead class="bg-slate-50">
          <tr>
              ...
              <th class="px-6 py-3 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider">Disponible
              </th>
              <th class="px-6 py-3 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider">Precio
                  Unitario</th>
              <th class="px-6 py-3 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider">Acciones
              </th>
          </tr>
      </thead>
      <tbody class="bg-white divide-y divide-gray-200">
          @forelse ($articulos as $articulo)
              @if ($articulo->nombre != 'Pago saldado')
                  <tr class="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium text-slate-900">
                          <span
                              class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $articulo->stock > 5 ? 'bg-slate-100 text-slate-800' : ($articulo->stock > 0 ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-rose-100 text-rose-800 border border-rose-300') }}"
                      ...
                      <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium">
                          <div class="flex items-center justify-center gap-2">
                              ...
                              <button type="button" onclick="openModal('edit-articulo-{{ $articulo->id }}')"
                                  ...
                                  class="inline-flex items-center justify-center text-indigo-600 hover:text-indigo-900 transition-colors cursor-pointer">
                              ...
                              {{-- Botón Eliminar --}}
                              <form id="delete-articulo-{{ $articulo->id }}" class="contents"
                                  ...
                                  <button type="submit" title="Eliminar Artículo"
                                      class="inline-flex items-center justify-center text-rose-600 hover:text-rose-900 transition-colors cursor-pointer">
                          ...
                          <x-modal name="edit-articulo-{{ $articulo->id }}">
                              <div class="p-6 text-left">
                                  <div class="flex justify-between items-center pb-3 border-b mb-4">
                                      <h3 class="text-lg font-bold text-gray-900">Editar Artículo:
                                          {{ $articulo->nombre }}</h3>
                                      <button type="button" onclick="closeModal('edit-articulo-{{ $articulo->id }}')"
          ...
          @empty
              <tr>
                  <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No hay artículos registrados.
                  </td>
              </tr>
          @endforelse
      </tbody>
          ...
          fetch(`/admin/articulos/${id}/toggle-disponible`, {
              ...
              .then(data => {
                  if (data.success) {
                      if (data.disponible) {
                          btn.className =
                              'p-2 inline-flex items-center justify-center rounded-full cursor-pointer transition-all duration-200 border shadow-sm bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100 hover:scale-110';
                          btn.title = 'Artículo Disponible - Clic para ocultar';
                          btn.innerHTML =
                              `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>`;
                      } else {
                          btn.className =
                              'p-2 inline-flex items-center justify-center rounded-full cursor-pointer transition-all duration-200 border shadow-sm bg-rose-50 border-rose-200 text-rose-600 hover:bg-rose-100 hover:scale-110';
