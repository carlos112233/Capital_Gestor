...
<div class="mb-4">
    <label for="tipo_pago" class="block text-gray-700 font-bold mb-2">Artículo</label>
    <select name="tipo_pago" id="tipo_pago" class="w-full border-gray-300 rounded-lg shadow-sm">
        <option value="">Seleccione un artículo</option>
        <option value="1">Por articulo</option>
        <option value="2">Saldar adeudo</option>

    </select>
</div>
<div class="grupo grupo-1" style="display:none;">
    <div class="mb-4">
...
<div class="grupo grupo-2" style="display:none;">
    <div class="mb-4">
        <label for="articulo_id" class="block text-gray-700 font-bold mb-2">Artículo</label>
        <select name="articulo_id" id="articulo_id" class="w-full border-gray-300 rounded-lg shadow-sm">
            <option value="">Seleccione un artículo</option>
            @foreach ($articulos as $articulo)
            @if($yarticulo->nombre == 'Saldar pago' )
                <option value="{{ $articulo->id }}"
                    {{ old('articulo_id', $venta->articulo_id ?? '') == $articulo->id ? 'selected' : '' }}>
                    {{ $articulo->nombre }}
                </option>
...
<script>
   const precios = {
    ...
    // Ocultar todos
    document.querySelectorAll('.grupo-1, .grupo-2').forEach(el => el.style.display = 'none');

    if (valor === '1') {
        document.querySelectorAll('.grupo-1').forEach(el => el.style.display = 'block');
    } else if (valor === '2') {
        document.querySelectorAll('.grupo-2').forEach(el => el.style.display = 'block');
    }
});
</script>